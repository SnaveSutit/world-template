...
function resolveEnvVariables(str: string): string {
	return str.replace(/%([^%]+)%|\$([A-Z_]+[A-Z0-9_]*)|\${([A-Z0-9_]*)}/gi, (_, a, b) => process.env[a || b] as string)
}
...
async function gatherMinecraftInstances() {
	const vanilla = Object.values(
		JSON.parse(await fs.readFile(pathjs.join(MINECRAFT_PATH, 'launcher_profiles.json'), { encoding: 'utf-8' }))
			.profiles
	).map((p: any) => {
		return {
			name: p.name,
...
async function main() {
	...
	const profileIndex = (await term.gridMenu(profiles[launcher].map((v: any) => v.name)).promise).selectedIndex
	...
	if (existsSync(saveLocation)) {
		term('\n')
			.brightRed(`A world named '${projectName}' already exists in the '${selectedProfile.name}' profile!`)
	...
	await fs.symlink(pathjs.join(process.cwd(), './datapacks'), dataPacksLocation, 'dir').catch(e => {})
	await fs.symlink(pathjs.join(process.cwd(), './world'), saveLocation, 'dir').catch(e => {})
	await fs.symlink(pathjs.join(process.cwd(), './resources'), resourcesLocation, 'dir').catch(e => {})
	...
	const metapaths = [
		pathjs.join(process.cwd(), './datapacks/World Template/pack.mcmeta'),
		pathjs.join(process.cwd(), './resources/pack.mcmeta'),
	]
	...
	await fs.rename(
		pathjs.join(process.cwd(), 'datapacks/World Template'),
		pathjs.join(process.cwd(), 'datapacks/' + projectName)
	)
